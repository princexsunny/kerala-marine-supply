// The Shalom ventures — the single source of truth for the homepage cards,
// the venture ring, the careers list and the individual venture pages.
//
// Order in this array IS the order shown, and `num` is the number printed on
// the card. Entries marked `hidden` stay here in full but appear nowhere:
// see KMS_VISIBLE at the foot of the file.
//
...
window.KMS_VENTURES = [
  {
    ...
    next:
      'Creates the demand signal the cold chain (08) and the export business (09) are built to ' +
      'serve.',
  },
  {
    ...
    brief:
      ...
      'It is live and taking orders, which makes it one of the first to prove the model.',
    opportunity:
    ...
    next:
      'Proves demand and builds the customer list that Shalom Marine Spare Parts (07) will sell ' +
      'into, and gives the machine workshop a route to market.',
  },
  {
    slug: 'marine-fuels',
    num: '03',
  ...
  },
  {
    ...
    jobs:
      'The largest employer of them all — 15 open roles across management, supervision, marine ' +
    ...
    next:
      'The yard is the keystone. It is what makes Shalom Shipbuilding (05) possible — building ' +
  ...
  },
  {
    ...
    next:
      'The fabrication capability supplies the yard and the shipbuilding venture with parts that ' +
      'would otherwise be bought in.',
  },
  {
    ...
    status: 'Building now',
    icon: 'prop',
    summary: 'Engines, pumps, propellers, generators, filters and other spare parts for marine and fishing vessels.',
    brief:
  ...
  },
  {
    ...
    status: 'Building now',
    icon: 'coldtruck',
    summary: 'Cold storage, ice plants, refrigerated transport and seafood logistics solutions.',
    brief:
  ...
  },
  {
    ...
    status: 'Next',
    icon: 'export',
    summary: 'Processing, packaging and export of high-quality seafood to global markets.',
    brief:
  ...
  },
  {
    ...
    next:
      'The habit and the trust it builds are what make Shalom Fish Online (01) plausible — a ' +
      'marketplace only works if the fishermen already have a reason to open your app.',
  },
  {
    // Not shown anywhere at the moment. Nothing here is deleted — remove this
    // line and give it a num to put it back in front of people.
    hidden: true,
    slug: 'fishing-gear',
    num: '07',
...
// The ventures actually shown to visitors, in order.
//
// Every page reads THIS, never the raw array — the home page cards, the ring,
// the careers list and the venture pages. That is the point: the cards used to
